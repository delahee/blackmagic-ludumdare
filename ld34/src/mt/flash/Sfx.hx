...
class Sfx {
	...
	//function pauseAfterDisable() {
		//if( isPlaying() ) {
			//resumeTime = curPlay.position;
			//stop();
		//}
	//}
//
	//function resumeAfterEnable() {
		//if( resumeTime>=0 ) {
			//play
		//}
	//}
	...
	public static function wipeChannel(channel:Int) {
		for( s in getByChannel(channel) )
			s.stop();
	}

	public static function wipeChannelWithFadeOut(channel:Int, fadeDuration:Float) {
		for( s in getByChannel(channel) )
			s.tweenVolume(0, fadeDuration).onEnd = function() {
				s.stop();
			}
	}
